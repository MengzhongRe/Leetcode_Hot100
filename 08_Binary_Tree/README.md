...
## 一、 LeetCode 236. 二叉树的最近公共祖先 (Medium)

本题是理解二叉树“自底向上”汇报机制的试金石。

### 1. 解题思路：后序遍历 + 状态汇报
要找“最近”的公共祖先，最自然的思路是从下往上找。我们在每个节点使用**后序遍历**（左右根），让左右子树先去寻找 $p$ 和 $q$，然后根据子树汇报的结果，当前节点做出决策。

对于当前节点 `root` 的左右返回值 `left` 和 `right`，有以下四种核心逻辑：
1. **`left` 和 `right` 都不为空**：说明 $p$ 和 $q$ 分别在当前节点的左右子树中。当前节点就是那个“分叉口”，即**最近公共祖先**，直接向上返回 `root`。
2. **`left` 为空，`right` 不为空**：说明左边没找到，目标全在右边。当前节点充当“传话筒”，直接把右边找到的东西（可能是目标节点，也可能是底下的祖先）原封不动传给上级，即 `return right`。
3. **`right` 为空，`left` 不为空**：同理，目标全在左边，充当传话筒，`return left`。
4. **都为空**：当前节点的地盘里啥也没有，如实汇报，`return None`。

### 2. 优化过程：绝妙的“剪枝”设计
本题代码极其精简，其中包含了一个非常精妙的隐性优化（剪枝）：
* **终止条件提前拦截**：`if not root or root == p or root == q: return root`。
* **原理解析**：当我们从上往下遍历，如果先碰到了 $p$，**代码会直接返回 $p$，而不再去遍历 $p$ 的左右子树！**
    * **情况 A ($q$ 在 $p$ 的下面)**：此时 $p$ 本身就是最近公共祖先，直接返回 $p$ 即为正确答案，省去了往下找 $q$ 的无效遍历。
    * **情况 B ($q$ 不在 $p$ 的下面)**：返回 $p$ 相当于向上级汇报“我在这一侧找到了 $p$”，后续会通过“传话筒”逻辑在更高的节点与 $q$ 汇合。
* **前置条件**：这个优化能成立，依赖于题目给定的隐藏金牌条件——**“$p$ 和 $q$ 必然存在于树中”**。如果题目不保证这一点，就不能提前剪枝，必须老老实实遍历全树。
...
class Solution:
    def lowestCommonAncestor(self, root: 'TreeNode', p: 'TreeNode', q: 'TreeNode') -> 'TreeNode':
        ...
        # 3. 处理返回值（汇报逻辑）
        if left and right:
            return root         # 左右各找到一个，我就是公共祖先
        
        return left if left else right  # 谁不为空返回谁，都为空返回 None
```

### 4. 简单复杂度分析
* **时间复杂度**：$O(N)$。最坏情况下（例如目标节点在树的最底层），需要遍历二叉树的所有节点，每个节点的操作为 $O(1)$ 常数时间。
* **空间复杂度**：$O(H)$。$H$ 为二叉树的高度。主要为递归调用栈的开销。最坏情况下（树退化为链表）为 $O(N)$，最好情况下（平衡二叉树）为 $O(\log N)$。

---

## 二、 LeetCode 124. 二叉树的最大路径和 (Hard)

如果说 236 题是“原样传话”，那么 124 题就是在传话时加了“两套账本”的变种。

### 1. 解题思路：两套账本 + 及时止损
题目要求找任意节点到任意节点的最大路径。由于路径不能走回头路（不能分叉），所以在每个节点处，我们必须将“全局最大路径（倒V型）”和“向上汇报的路径（单链型）”区分开来。

* **账本一：算私房钱（更新全局最大值）**
  * 路径可以跨越当前节点，形成“左子树单链 + 当前节点 + 右子树单链”的**倒V型**。
  * 动作：在每个节点，计算这个倒V型总和，并尝试刷新全局变量 `self.max_sum`。
* **账本二：向上级汇报（函数的返回值）**
  * 当前节点只能提供**一条**不分叉的路径给父节点。
  * 动作：当前节点值 + `max(左边单链, 右边单链)`。
* **核心技巧：及时止损（处理负数）**
  * 如果左或右子树汇报上来的单链收益是负数，接上它只会越加越小，我们宁可不要！
  * 动作：`max(dfs(node), 0)`，如果是负数直接按 0 算，相当于从这里“砍断”路径。
...
class Solution:
    ...
    def maxPathSum(self, root: 'TreeNode') -> int:
        ...
        # 递归函数：返回以当前节点为顶点的最大【单链】和
        def dfs(node):
            if not node:
                return 0
            
            # 及时止损：如果子树提供的是负收益，直接当 0 处理（不走那条路）
            left_gain = max(dfs(node.left), 0)
            right_gain = max(dfs(node.right), 0)
            
            # 账本一：算私房钱（倒V型），尝试更新全局最大值
            current_path_sum = node.val + left_gain + right_gain
            self.max_sum = max(self.max_sum, current_path_sum)
            
            # 账本二：向上级汇报，只能挑左右两边收益最大的一条【单链】报上去
...
### 3. 简单复杂度分析
* **时间复杂度**：$O(N)$。同样是后序遍历（DFS），每个节点只会被访问一次，每次访问进行比较和加法等常数级别 $O(1)$ 的操作。
* **空间复杂度**：$O(H)$。$H$ 为二叉树的高度。空间消耗依然全在于递归调用栈的深度。最坏为链表退化 $O(N)$，最好为平衡树 $O(\log N)$。