...
- 有环：快慢指针最终进入环，快指针速度更快，必然在环内追上慢指针（相遇），返回 `true`。并且一定是在慢指针绕环的第一个周期即会与快指针相遇！

#### 数学证明（简单版）
设环长为 `L`，慢指针入环时，快慢指针的环内距离为 `d`。`d`最大为`L - 1`,即慢指针在入环点，快指针在下一个节点。每轮移动后，快指针比慢指针多走 1 步，距离减少 1，经过 `d` 轮，即最大为`L - 1`轮后后距离为 0，必然相遇。经过`L - 1`轮后慢指针刚好在环终点，因此慢指针一定会在第一个绕环周期内娱快指针相遇。
